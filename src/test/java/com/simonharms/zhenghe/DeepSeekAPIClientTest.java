...
class DeepSeekAPIClientTest {
    ...
    @Test
    void sendStreamingPostRequest_setsAcceptEventStreamHeader()
        throws Exception {
    ...
    void sendStreamingPostRequest_serverError_throwsIOException() {
        server.enqueue(
            new MockResponse().setResponseCode(500).setBody("error")
        );
    ...
    @Test
    void sendStreamingPostRequest_deliversAllChunksToConsumer()
        throws Exception {
        // Role-announcement chunk has no content — the consumer decides what to process
        String sseBody =
        ...
        assertEquals(
            2,
            callCount.get(),
            "Consumer should be called for every parsed chunk"
        );
